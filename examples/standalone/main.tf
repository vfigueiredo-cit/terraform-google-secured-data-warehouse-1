...
module "regional_deid" {
  ...
  parameters = {
    ...
    outputBigQueryTable    = "${module.base_projects.non_confidential_data_project_id}:${local.non_confidential_dataset_id}.${trimsuffix(local.cc_file_name, ".csv")}"
    bqProjectId            = module.base_projects.non_confidential_data_project_id
    datasetName            = "non_confidential_dataset"
    batchSize              = 1000
...
module "regional_reid" {
  ...
  parameters = {
    ...
    outputBigQueryTable       = "${module.base_projects.confidential_data_project_id}:${local.confidential_dataset_id}.${trimsuffix(local.cc_file_name, ".csv")}"
